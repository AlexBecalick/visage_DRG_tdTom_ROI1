function add_spots_patched(all_geneData, map) {
    ...
    function generateCircleTexture(color, radius, renderer) {
        ...
        gfx.alpha = 0.8; // the opacity of the gene markers
    ...
    function scaleRamp(z) {
        ...
        return z === 0 ? scaleRampHelper(z, scale) : //
                z === 1 ? scaleRampHelper(z,  2 * scale) :
                    z === 2 ? scaleRampHelper(z,  2 * scale) : scaleRampHelper(z, 4 * scale) //scaleRampHelper is
    ...
    function scaleRampHelper(z, scale){
        // makes a tiny dot and scales it up based on the map and the dapi dimensions
        ...
        // this function is lets say 10, then to keep the same size on the screen for the dot, at zoom = 4
        ...
        var map_side = mapSide(configSettings.zoomLevels),
            ...
            max_dapi = Math.max(...dapi_size), // this is the max dimension of the dapi
            c = map_side / max_dapi, // this is the ratio between the map and the dapi
            tiny_dot = 1 / (1.7**z), // this is the size of the dot at zoom level 0
            dot = c * tiny_dot; // this is the size of the dot at the current zoom level
        return dot * scale
    }
        ...
        return L.pixiOverlay(function (utils, event) {
            ...
            if (firstDraw) {
                if (event.type === 'add') {
                    ...
                    initialScale = invScale / 32;
                    // initialScale = 0.0125;
                    var targetScale = zoom <= zoomSwitch ? scaleRamp(zoom) : 1; // the zoomSwitch is the zoom level at which we switch from the scaleRamp to the scaleRampHelper
