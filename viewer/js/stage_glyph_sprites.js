function drawGlyphSprites(resources, markers, map) {
    ...
    var pixiLayer = glyphOverlay(markers, textures, focusTextures);
    // pixiLayer().addTo(map);
    return pixiLayer()
}

function glyphOverlay(markers, textures, focusTextures) {
    return function () {
    ...
    function drawCallback(markers, textures, focusTextures) {
        ...
        return function (utils, event) {
            ...
            if (firstDraw) {
                ...
                let zoomQuadTree = {};
                for (var z = 7; z <= map.getMaxZoom(); z++) {
            ...
            if ((firstDraw || prevZoom !== zoom) && (zoom >=7)) {
